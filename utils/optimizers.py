...
def get_optimizer(model, encoder_name, learning_rate, weight_decay, momentum, clipping=False, lars=False):
    if encoder_name == 'WideResNet' and lars is False:
        return optim.SGD(model.parameters(), lr=learning_rate, weight_decay=weight_decay, momentum=momentum)
    if encoder_name == 'WideResNet' and lars is True:
        return LARSWrapper(
            optim.SGD(model.parameters(), lr=learning_rate, weight_decay=weight_decay, momentum=momentum))
