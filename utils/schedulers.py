...
def get_scheduler(optimizer, encoder_name='WideResNet', epochs=500, cosine=False,
                  warmup_epochs=0, decay_epochs=None):
    if decay_epochs is None:
        decay_epochs = [5, 30, 50, 80]

    if encoder_name == 'WideResNet' and cosine is True and warmup_epochs > 0:
        return LinearWarmupCosineAnnealingLR(optimizer=optimizer, warmup_epochs=warmup_epochs, max_epochs=epochs,
                                             eta_min=optimizer.param_groups[0]['lr'] * (0.1 ** 3))
    if encoder_name == 'WideResNet' and cosine is True and warmup_epochs == 0:
        return MultiStepLR(optimizer, milestones=decay_epochs, gamma=0.1)

    if encoder_name == 'ResNet50' and cosine is True:
        return LinearWarmupCosineAnnealingLR(optimizer=optimizer, warmup_epochs=warmup_epochs, max_epochs=epochs,
                                             eta_min=optimizer.param_groups[0]['lr'] * (0.1 ** 3))
